	...
	for (int i = 0; i < rows_num; ++i)
	{
		...
		for (int j = 0; j < rows_num; ++j)
		{
			if ((i == 0) || (j == 0) || (i == rows_num - 1) || (j == rows_num - 1) ||
				...
				cells[i][j] = std::make_shared<StaticCell>(QRectF(0, 0, cell_side_size, cell_side_size), cell_corner_radius, this);
			else
			{
			...
			//
			selections[i][j] = std::make_shared<Selection>(QRectF(0, 0, cell_side_size, cell_side_size), cell_corner_radius, this);
			selections[i][j]->setPos(get_cell_center(j, i));
		}
	}
...
void GridMap::select_cell(size_t _x, size_t _y)
{

}

void GridMap::deselect_cell(size_t _x, size_t _y)
{
